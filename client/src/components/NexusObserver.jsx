...
import reinaMobile from '../assets/reina-mobile.webp';
import historyScreenshot from '../assets/history-log.webp'; 
...
const NexusObserver = () => {
  ...
  return (
    <section className="relative w-full max-w-7xl mx-auto px-6 py-32 flex flex-col items-center border-t border-white/5 relative overflow-hidden">
      ...
      {/* Background Glow */}
      <div className={`absolute top-[30%] left-[-20%] w-[600px] h-[600px] bg-rose-900/10 ${isMobile ? 'blur-[50px] opacity-60' : 'blur-[130px]'} rounded-full -z-10`} />
      ...
      >
        ...
        <div className="relative w-full h-full bg-[#050505] rounded-2xl overflow-hidden border border-white/5">
          {isMobile ? (
            <img 
              src={reinaMobile} 
              alt="Astrix AI Presence Reina" 
              loading="lazy"
              className="w-full h-full object-cover scale-[1.03] group-hover:scale-[1.08] transition-transform duration-1000 ease-out opacity-90 group-hover:opacity-100"
            />
          ) : (
            <video 
              src={isVideoVisible ? reinaVideo : undefined} 
              autoPlay 
              loop 
              muted 
              playsInline 
      ...
      <div className="w-full flex flex-col lg:flex-row items-center gap-16 z-10">
        ...
        >
          <motion.div 
            ...
            className={`relative w-full rounded-2xl p-2 bg-gradient-to-tr from-rose-500/10 to-transparent border border-rose-500/20 ${isMobile ? '' : 'rotate-y-[-5deg] rotate-x-[5deg]'} transition-all duration-700 ${isMobile ? 'shadow-[0_15px_30px_rgba(0,0,0,0.6)]' : 'shadow-[20px_30px_70px_rgba(225,29,72,0.15),0_0_40px_rgba(0,0,0,0.8)]'}`}
          >
